...
class EditDialogue extends StatefulWidget {
  ...
  const EditDialogue({
    ...
    required this.price,
    required this.resturants, required this.des, required this.imgUrl, required this.id,
  }) : super(key: key);
  @override
...
class _EditDialogueState extends State<EditDialogue> {
  ...
  Future save() async {
    if (pick == 1) {
      ...
      if (addImg.state == TaskState.success) {
        ...
        FirebaseFirestore.instance.collection('dish').doc(widget.id).update({
  ...
  Future del()async{
    await FirebaseFirestore.instance.collection('dish').doc(widget.id).delete();
  }
}
